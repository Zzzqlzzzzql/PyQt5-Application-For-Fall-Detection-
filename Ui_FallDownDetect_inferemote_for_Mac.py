...
class Ui_MainWindow(object):
    ...
    def open_file(self):
        self.download_path = QtWidgets.QFileDialog.getOpenFileName(
            self,
            "选择图片或视频",
            "C:/Users/Zzzql/Desktop/images",
            "Image Files (*.jpg *.png);;Video Files (*.mp4 *.mov)",
        )
        if not self.download_path[0].strip():
            ...
            if self.current_input == "picture":
                ...
                if pic is not None:
                    ...
                    QtWidgets.QMessageBox.warning(self, "警告", "无法读取图片文件。")
            elif self.current_input == "video":
                ...
                if cap.isOpened():
                    ...
                    QtWidgets.QMessageBox.warning(self, "警告", "无法打开视频文件。")
            elif self.current_input == "camera":
                pass  # 可选择添加提示图片或信息
            self.Log("准备运行")
    ...
    def process_image(self):
        self.Log("开始测试")
        # 缓存图片为指定路径
        cache_image_path = "PYQT/cache.jpg"
        try:
        ...
        if os.path.exists(output_image_path):
            ...
            QtWidgets.QMessageBox.warning(
                self, "警告", f"结果图片未找到: {output_image_path}"
            )
            self.Log("结果图片未找到。")
        self.Log("测试完成")
    ...
    def process_camera(self):
        ...
        script_path = os.path.join(
            "/Users/everlasting/Downloads/fall-detection/inferemote", "test.py"
        )  # 确保test.py的路径正确

        command = f'''
        source /Users/everlasting/miniconda3/etc/profile.d/conda.sh
        conda activate py39
        python "{script_path}" -r localhost -p 9023 -i camera
        '''
    ...
    def run_inferemote(self, cv2likeimage):
        ...
        command = [
            "python",
            "F:\\GitHub\\fall-detection\\inferemote\\test.py",
            "-r",
            "localhost",
            "-p",
            "9023",
